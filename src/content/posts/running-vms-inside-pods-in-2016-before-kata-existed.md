...
The next post leaves the control plane and the runtime behind and goes down to the packets themselves. When the company tried to reinvent itself as a 5G-edge box, "good enough" networking through the kernel suddenly wasn't good enough: edge means line-rate on commodity hardware, and our data path topped out around 12 Gbps on 100 Gbps cards. Taking it out of the kernel with DPDK, and getting to roughly 60 Gbps on the same servers, is where the series goes next.
